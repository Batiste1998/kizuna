...
(() => {
  ...
  const stylesheet =
    ...
    // Initials fallback: when a `fallback` attr is present the empty state
    // shows a filled monogram (avatar) instead of the drop icon. The whole
    // empty layer stays clickable + droppable so the user can still add a photo.
    ...
    '.empty.fb-on{padding:0}' +
    '.empty.fb-on>svg,.empty.fb-on .cap,.empty.fb-on .sub{display:none}' +
    '.empty.fb-on .fb{display:flex}' +
    ':host([data-editable]) .empty.fb-on:hover .fb{filter:brightness(.96)}' +
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'position', 'placeholder', 'src', 'id',
        'fallback', 'fallback-bg', 'fallback-color'];
    }
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="fb" part="fallback"></div></div>' +
    ...
    _render() {
      ...
      if (url) {
        ...
        if (fb) {
          this._empty.classList.add('fb-on');
          this._fb.textContent = fb;
          this._fb.style.background = this.getAttribute('fallback-bg') || 'rgba(0,0,0,.06)';
          this._fb.style.color = this.getAttribute('fallback-color') || 'rgba(0,0,0,.55)';
          const w = this.clientWidth || 40;
          this._fb.style.fontSize = Math.max(10, Math.round(w * 0.4)) + 'px';
          this._ring.style.display = 'none';
        } else {
          this._empty.classList.remove('fb-on');
        }
      }
    }
  }
